      ...
      if (compiler.hooks) {
        (function () {
          ...
          compiler.hooks.normalModuleFactory.tap(plugin, function (nmf) {
            if (isWebpack5) {
              nmf.hooks.afterResolve.tapAsync(plugin, function (result, callback) {
          ...
          if (isWebpack5) {
            compiler.hooks.thisCompilation.tap(plugin, function (compilation) {
              compilation.hooks.processAssets.tapAsync({ name: plugin.name, stage: compiler.webpack.Compilation.PROCESS_ASSETS_STAGE_SUMMARIZE }, function (result, callback) {
                return emitFn(compilation, callback);
              });
            });
          } else {
